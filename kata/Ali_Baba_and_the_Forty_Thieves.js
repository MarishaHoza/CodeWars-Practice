...
//If he opens the door successfully but he did not close the door:
// Return the forecast result:
...
function aliBaba(trying){
    let openPassword = "Open, Sesame!";
    let closePassword = "Shut, Sesame!";
    let open = false;
    let close = false;
  
    for (var i=0; i<trying.length; i++){
      if (trying[i] === openPassword){
        open = true;
      } else if (trying[i] === closePassword && open === true){
        close = true;
      }
    }
  ...
  aliBaba(["Open, Millet!","Open, Maize!","Open, Soybean!",
